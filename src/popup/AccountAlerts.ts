...
import {
  AccountPageStoreType,
  parseHealth,
  parseBalance,
  TriggeredAccountAlerts,
} from "./AccountPage";
...
export default () => ({
  ...
  getAccountName(selectedAccount: string): string {
    ...
    debug(
      "selected account: ",
      AccountPageStore.selectedAccount,
    ...
    if (selected) {
      ...
      shortAddress =
        AccountPageStore.selectedAccount.substring(0, 4) +
        "..." +
        AccountPageStore.selectedAccount.substring(length - 4);
      if (selected.name) {
  ...
  getAccountHealth(selectedAccount: string): number | ">100" {
    // selected = UserDataStore.accounts[AccountPageStore.selectedAccount]
    selected = UserDataStore.accounts[selectedAccount];
    return selected ? parseHealth(selected.health) : undefined
    },
  getAccountBalance(selectedAccount: string): string {
    // selected = UserDataStore.accounts[AccountPageStore.selectedAccount]
    selected = UserDataStore.accounts[selectedAccount];
    return selected ? parseBalance(selected.balance) : undefined
  },
  ...
  formatTriggerValueForDisplay(alert: AccountAlert) {
    return alert.triggerType === TriggerType.Static
      ? `$${alert.triggerValue}`
      : `${alert.deltaValue}%`;
  },
  formatDeltaValueForDisplay(alert: AccountAlert) {
    return alert.triggerType === TriggerType.Delta
      ? `${alert.timeFrame} hrs`
      : "n/a";
  },
